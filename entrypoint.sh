...
if [ "${INPUT_ONLY_CHANGED_FILES}" = "true" ]; then
    ...
    ${INPUT_PHPCS_BIN_PATH} ${ENABLE_WARNINGS_FLAG} --report=checkstyle | filter-by-changed-lines "$(git diff -U0 "${COMPARE_FROM_REF}" "${COMPARE_TO_REF}" | diff-lines | grep -ve ':-' | sed 's/:\+.*//')"
fi
